...
to setup
  ...
  ; initialize the board. We use _ to indicate the space is empty.
  set current_board (list  ( list "_" "_" "_" )
                           ( list "_" "_" "_" )
                           ( list "_" "_" "_" )  )

  ; You might want to use these two lines to help debug your code:
...
;;;;;;;;;;;;;;;;;
;; Calls minimax to find the best move  ---  minimax has been implemented underneath
...
;;;;;;;;;;;;;;
;; The minimax function section will return a list containing the x,y position of the best move discovered by the
;;    minmax algorithm for the board
;; The board setting is a 2D list that contains single character strings, the "_" is used to indicate empty locations

to-report minimax [board]
  report max_value board 0
end
...
to-report max_value [board depth]
  ...
  let value [-1 -1 -1000]
  foreach (actions board) [move ->
    set [x y] move
    set result replace-in-board x y board "x"
    set score min_value result (depth + 1)
    if (item 2 score) > (item 2 value) [set value (list x y (item 2 score))]
  ]
  report value
end
...
to-report min_value [board depth]
  ...
  foreach (actions board) [move ->
    set [x y] move
    set result replace-in-board x y board "o"
    set score max_value result (depth + 1)
    if (item 2 score) < (item 2 value) [set value (list x y (item 2 score))]
  ]
  report value
end
...
to-report actions [board]
  let [available x y] [[] 0 0]
  repeat 3 [
    repeat 3 [
      if item x (item y board) = "_" [
        set available lput (list x y) available
      ]
      set x x + 1
    ]
    set x 0
...
to-report evaluate [board]
  ...
  ; Else if neither of them has won, then return 0
  report 0
end
...
to-report calculate-utility [score depth]
  ...
  if (score = -10) [ report score + depth]
  ; Otherwise, return 0 if there's no winner yet
  report 0
end
;>========================================================================
@#$#@#$#@
